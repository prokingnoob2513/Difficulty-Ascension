...
function fixValue(x, y = 0) {
    return x || new MegotaNum(y)
}

function sumValues(x) {
    x = Object.values(x)
    if (!x[0]) return new MegotaNum(0)
    return x.reduce((a, b) => MegotaNum.add(a, b))
}

function format(decimal, precision = 2, small=false) {
    small = small || modInfo.allowSmall
    decimal = new MegotaNum(decimal)
...
function toPlaces(x, precision, maxAccepted) {
    ...
    if (new MegotaNum(result).gte(maxAccepted)) {
        result = new MegotaNum(maxAccepted - Math.pow(0.1, precision)).toString(precision)
    }
    return result
}